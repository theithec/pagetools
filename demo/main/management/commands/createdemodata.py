...
def create():
    ...
    for d in pages_data:
        ...
        if len(d) > 3:
            kwargs = d[3]
        page = Page.objects.get_or_create(title=d[0], slug=d[1], content=d[2], status=STATUS_PUBLISHED, **kwargs)[0]
    ...
    for d in widgets_areas:
        WidgetInArea.objects.create(typearea=d[1], content_object=d[0], position=0, enabled=True)
