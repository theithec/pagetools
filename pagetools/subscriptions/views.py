    ...
    if form.is_valid():
        ...
        if not already_there:
            subscriber = Subscriber(
                email=email, is_activated=False, lang=get_language()
            )
...
def activate(request, key):
    ...
    if subscriber and not subscriber.is_activated and activate_end > timezone.now():
        ...
        return render(
            request, subs_settings.MSG_BASE_TEMPLATE, {"msg": _("activation: ok")}
        )
    raise Http404()
...
def unsubscribe(request, key):
    ...
    if subscriber and subscriber.is_activated:
        ...
        return render(
            request, subs_settings.MSG_BASE_TEMPLATE, {"msg": _("unsubscribe: ok")}
        )
    raise Http404()
