...
class WidgetTests(TestCase):
    def setUp(self):
        ...
        self.typearea1 = TypeArea.objects.get_or_create(area="sidebar", pagetype=self.type1)[0]

    def test_model_creation(self):
        widget = ContentWidget.objects.get_or_create(name="w1", content="foo")[0]
        ctype = ContentType.objects.get_for_model(widget)
        WidgetInArea.objects.get_or_create(typearea=self.typearea1, content_type=ctype, object_id=widget.pk, position=1)
