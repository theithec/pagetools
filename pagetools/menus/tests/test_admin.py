...
class MenuAdminTests(TestCase):
    ...
    def test_reorder(self):
        ...
        data = self._data_from_menu(menu)
        self.client.post(adminurl, data)
        self.assertEqual([entry['entry_title'] for entry in menu.children_list()],
                         ['e1', 'e2'])
        data.update({
            'entry-order': "[%s]=null&[%s]=null" % (entries[1].pk, entries[0].pk),
        })
        self.client.post(adminurl, data)
        self.assertEqual([e['entry_title'] for e in menu.children_list()],
                         ['e2', 'e1'])
    ...
    def test_addentry(self):
        ...
        for i in range(1, 3):
            ...
            adminurl = reverse(
                'admin:menus_menu_change', args=[menu.pk])
            data = self._data_from_menu(menu)
